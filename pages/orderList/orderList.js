...
Page({
  ...
  onLoad: function(options) {
    const userId = wx.getStorageSync('userId'); // 假设用户ID存储在本地
    if (options.sts) {
      this.setData({
        sts: options.sts
      });
      this.loadOrderData(userId, options.sts, 1);
    } else {
      this.loadOrderData(userId, 0, 1);
    }
  },
   ...
   * 加载订单数据
   */
  loadOrderData: function(userId, sts, current) {
    var ths = this;
    wx.showLoading();

    var url = `/order/v1/list?userId=${userId}&isDelete=0`;
    if (sts != 0) {
      url += `&status=${sts}`;
    }
    //加载订单列表
  ...
  onStsTap: function(e) {
    ...
    this.data.list = [];
    this.loadOrderData(wx.getStorageSync('userId'), sts, 1);
  },
  ...
  onReachBottom: function() {
    if (this.data.current < this.data.pages) {
      this.loadOrderData(wx.getStorageSync('userId'), this.data.sts, this.data.current + 1);
    }
  },
      ...
      success(res) {
        if (res.confirm) {
          ...
          var params = {
            ...
            callBack: function(res) {
              //console.log(res);
              ths.loadOrderData(wx.getStorageSync('userId'), ths.data.sts, 1);
              wx.hideLoading();
            }
          };
          http.request(params);
